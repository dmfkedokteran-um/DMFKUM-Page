function B(){const T=localStorage.getItem("isAdmin")==="true",w=localStorage.getItem("dmfk_settings");if(w)try{const e=JSON.parse(w);if(e.visi){const a=document.getElementById("profile-visi-text");a&&(a.textContent=e.visi)}if(e.misi&&Array.isArray(e.misi)){const a=document.getElementById("profile-misi-list");a&&(a.innerHTML=e.misi.map(l=>`
...
`).map(r=>r.trim()).filter(r=>r);await k({visi:a,misi:i}),p.close(),window.location.reload()});async function c(e,a){const l=e?.files?.[0];return l?new Promise(i=>{const r=new FileReader;r.onload=()=>i(r.result),r.onerror=()=>i(a),r.readAsDataURL(l)}):a}const L=document.getElementById("edit-org-bg-btn"),g=document.getElementById("org-bg-modal"),$=document.getElementById("close-org-bg-modal-btn"),K=document.getElementById("org-bg-form");L&&g&&L.addEventListener("click",()=>g.showModal()),$&&g&&$.addEventListener("click",()=>g.close()),K&&g&&K.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("parliament-name-input")?.value.trim(),l=document.getElementById("parliament-logo-url-input")?.value.trim(),i=document.getElementById("parliament-logo-file-input"),r=await c(i,l),u=document.getElementById("org-title-input")?.value.trim(),f=document.getElementById("org-subtitle-input")?.value.trim(),h=document.getElementById("org-bg-url-input")?.value.trim(),v=document.getElementById("org-bg-file-input"),y=await c(v,h);await k({parliamentName:a||"Parlemen Nawa Cita 2026",parliamentLogo:r,orgTitle:u||"Struktur Kepengurusan 2026",orgSubtitle:f||"Pimpinan Utama, BPH, dan Komisi Operasional Organisasi DMFK UM",orgBgImage:y}),g.close(),window.location.reload()});const q=document.getElementById("edit-org-btn"),b=document.getElementById("org-modal"),N=document.getElementById("close-org-modal-btn"),n=document.getElementById("org-form");q&&b&&q.addEventListener("click",()=>{s(),b.showModal()}),N&&b&&N.addEventListener("click",()=>b.close());let o=n?JSON.parse(n.getAttribute("data-org")||"{}"):{};function s(){if(!n)return;const e=document.getElementById("presidium-inputs-container");e&&(e.innerHTML=(o.presidium||[]).map((i,r)=>`
        ...
        `).join(""))}window.removePresidiumItem=e=>{o.presidium.splice(e,1),s()},window.removeSekbenItem=e=>{o.sekben.splice(e,1),s()},window.removeKomisiItem=e=>{o.komisi.splice(e,1),s()},window.addAnggotaItem=e=>{o.komisi[e].anggota||(o.komisi[e].anggota=[]),o.komisi[e].anggota.push({name:"",dept:"",avatar:""}),s()},window.removeAnggotaItem=(e,a)=>{o.komisi[e].anggota.splice(a,1),s()},document.getElementById("add-presidium-btn")?.addEventListener("click",()=>{o.presidium||(o.presidium=[]),o.presidium.push({role:"Wakil Ketua",name:"",dept:"",avatar:""}),s()}),document.getElementById("add-sekben-btn")?.addEventListener("click",()=>{o.sekben||(o.sekben=[]),o.sekben.push({role:"Sekretaris",name:"",dept:"",avatar:""}),s()}),document.getElementById("add-komisi-btn")?.addEventListener("click",()=>{o.komisi||(o.komisi=[]);const e=o.komisi.length+1;o.komisi.push({id:`komisi-${e}`,code:`Komisi ${e}`,title:`Komisi Baru ${e}`,themeColor:e%2===0?"blue":"rose",ketua:{name:"",dept:"",avatar:""},anggota:[{name:"",dept:"",avatar:""}]}),s()}),n&&b&&n.addEventListener("submit",async e=>{e.preventDefault();const a=document.getElementById("pembina-title")?.value.trim()||"Pembina Organisasi",l=document.getElementById("pembina-name")?.value.trim()||"",i=document.getElementById("pembina-nip")?.value.trim()||"",r=document.getElementById("pembina-avatar")?.value.trim()||"",u=document.getElementById("pembina-file"),f=await c(u,r||o.pembina?.avatar||"https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&q=80"),h={title:a,name:l,nip:i,avatar:f},v=n.querySelectorAll(".pres-role"),y=n.querySelectorAll(".pres-name"),D=n.querySelectorAll(".pres-dept"),R=n.querySelectorAll(".pres-avatar"),j=n.querySelectorAll(".pres-file"),P=[];for(let t=0;t<v.length;t++)if(y[t]?.value.trim()){const d=await c(j[t],R[t]?.value.trim()||o.presidium?.[t]?.avatar||"https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?auto=format&fit=crop&w=150&q=80");P.push({role:v[t].value.trim(),name:y[t].value.trim(),dept:D[t]?.value.trim()||"",avatar:d})}const F=n.querySelectorAll(".sb-role"),O=n.querySelectorAll(".sb-name"),J=n.querySelectorAll(".sb-dept"),H=n.querySelectorAll(".sb-avatar"),V=n.querySelectorAll(".sb-file"),C=[];for(let t=0;t<F.length;t++)if(O[t]?.value.trim()){const d=await c(V[t],H[t]?.value.trim()||o.sekben?.[t]?.avatar||"https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=150&q=80");C.push({role:F[t].value.trim(),name:O[t].value.trim(),dept:J[t]?.value.trim()||"",avatar:d})}const I=n.querySelectorAll(".kom-code"),_=n.querySelectorAll(".kom-title"),W=n.querySelectorAll(".kom-ketua-name"),z=n.querySelectorAll(".kom-ketua-dept"),G=n.querySelectorAll(".kom-ketua-avatar"),Q=n.querySelectorAll(".kom-ketua-file"),M=[];for(let t=0;t<I.length;t++){const d=I[t].closest(".bg-white"),x=d?.querySelectorAll(".kom-mem-name"),X=d?.querySelectorAll(".kom-mem-dept"),Y=d?.querySelectorAll(".kom-mem-avatar"),Z=d?.querySelectorAll(".kom-mem-file"),ee=await c(Q[t],G[t]?.value.trim()||o.komisi?.[t]?.ketua?.avatar||"https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=150&q=80"),U=[];if(x){for(let m=0;m<x.length;m++)if(x[m].value.trim()){const te=await c(Z[m],Y[m]?.value.trim()||o.komisi?.[t]?.anggota?.[m]?.avatar||"https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=150&q=80");U.push({name:x[m].value.trim(),dept:X?.[m]?.value.trim()||"",avatar:te})}}M.push({id:o.komisi?.[t]?.id||`komisi-${t+1}`,code:I[t].value.trim(),title:_[t]?.value.trim()||"",themeColor:o.komisi?.[t]?.themeColor||(t%2===0?"rose":"blue"),ketua:{name:W[t]?.value.trim()||"",dept:z[t]?.value.trim()||"",avatar:ee},anggota:U})}await k({orgStructure:{pembina:h,presidium:P,sekben:C,komisi:M}}),b.close(),window.location.reload()})}B();document.addEventListener("astro:after-swap",B);document.addEventListener("astro:page-load",B);
